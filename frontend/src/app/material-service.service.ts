...
solicitarMaterial(materialId: number, usuarioId: number): Observable<string> {
  //return this.httpClient.post(`${this.baseURL}/materiales/${materialId}/solicitar/${usuarioId}`, {}, { responseType: 'text' });
  const url = `http://localhost:8080/api/materiales/${materialId}/solicitar/${usuarioId}`;
  return this.httpClient.post(url, null, { responseType: 'text' });
}

  // Obtener solicitudes pendientes
  
  obtenerSolicitudesPendientes(): Observable<Solicitud[]> {
    return this.httpClient.get<Solicitud[]>(`${this.baseURL}/solicitudes-pendientes`);
  }

  //Confirmar o rechazar solicitud
  confirmarSolicitud(solicitudId: number, aprobado: boolean): Observable<string> {
    return this.httpClient.post(`${this.baseURL}/confirmar-solicitud/${solicitudId}?aprobado=${aprobado}`, {}, { responseType: 'text' });
  }
}
